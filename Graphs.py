...
print('Number of agents who are over break compliance:', len(over_comp))
over_comp.head(len(over_comp))
...
# Sort by highest Case Work duration
case_work = case_work.sort_values('Duration', ascending=False)
...
# Create dataframe of agents who are spending more than 10% of their time in
# Case Work
high_cw = case_work.where(case_work['Percent'] >= 10)
high_cw = high_cw.dropna()
high_cw = high_cw.sort_values('Percent', ascending=False)
print('Number of agents who spend more than 10% of their time in Case Work:',
      len(high_cw))
high_cw.head(len(high_cw))
